...
TutorialExtensionTestModelIndex.HasJpgScreenshot = [
  // List of only models that have *.jpg screenshots
  'AttenuationTest',
  'ClearCoatTest',
  'DragonAttenuation',
  'GlamVelvetSofa',
...
TutorialExtensionTestModelIndex.getCurrentModel = function () {
  ...
  if (options.model && modelInfoCollection.hasOwnProperty(options.model)) {
    ...
    if (options.type == 'glTF-Binary' || (options.type == 'glTF-Draco' && options.model == 'MorphPrimitivesTest')) {
      modelInfoCollection[options.model].path =
        modelInfoCollection[options.model].name +
        '/' +
        options.type +
        '/' +
        modelInfoCollection[options.model].name +
...
export default TutorialExtensionTestModelIndex
